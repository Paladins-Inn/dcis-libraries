...
class DetainedUserTest {
  ...
  private UserState generateUser() {
    ...
    Mockito.when(mockUser.isDetained()).thenReturn(true);
    Mockito.when(mockUser.isDeleted()).thenReturn(false);
    
    return new DetainedUser(mockUser, mockBus);
  }
  
  @Test
  void shouldReturnDetainedUserWhenUserIsActivated() {
    UserState result = sut.activate();

    assertTrue(DetainedUser.class.isAssignableFrom(result.getClass()), "Class is of type "+ result.getClass().getCanonicalName());
  ...
  void shouldReturnBannedUserWhenUserIsBanned() {
    ...
    verify(mockBus, times(1)).post(any(UserBannedEvent.class));
  }
